...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `Insert_interview`(
in p_ExecuteType  varchar(20),
out p_Result  varchar(20),
in p_InterviewID int(11) ,
in p_ApplicationID int(11) ,
...
BEGIN
set p_Result=0;
IF (select count(*) from interview  where ApplicationID = p_ApplicationID) = 0
then
insert into interview ( ApplicationID,InterviewerUserID,isMandatory,isEvaluator,isInActive,SessionID,TransactionKey,UpdatedBy,UpdatedOn,Description)
values(
p_ApplicationID,p_InterviewerUserID,p_isMandatory,p_isEvaluator,p_isInActive,p_SessionID,p_TransactionKey,p_UpdatedBy,p_UpdatedOn,p_Description);
set p_Result=200;
else
set p_Result=300;
end if;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `Update_interview`(
in p_ExecuteType  varchar(20),
out p_Result  varchar(20),
in p_InterviewID int(11) ,
in p_ApplicationID int(11) ,
...
BEGIN
set p_Result=0;
IF (select count(*) from interview  where (ApplicationID = p_ApplicationID) and (InterviewID <> p_InterviewID))  = 0
then
update interview set  ApplicationID = p_ApplicationID,InterviewerUserID = p_InterviewerUserID,isMandatory = p_isMandatory,isEvaluator = p_isEvaluator,isInActive = p_isInActive,SessionID = p_SessionID,TransactionKey = p_TransactionKey,UpdatedBy = p_UpdatedBy,UpdatedOn = p_UpdatedOn,Description = p_Description 
where InterviewID = p_InterviewID ; 
set p_Result=200;
else
set p_Result=300;
...
-- Dump completed on 2018-08-14 15:20:00
